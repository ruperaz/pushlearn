...
class CardAnswer extends Component {
    ...
    componentDidMount() {
        if(this.props.category){
            this.props.fetchNextTodayCard(this.props.category.details.id);
        }else{
    ...
    onClickAnswerBtn = (value) => {
        ...
        let variant="";
        if (value) {
            card={id: this.props.nextTodayCard.details.id , card_level: this.props.nextTodayCard.details.card_level + 1};
            message="Card successfully added to Next Level";
            variant="success";
        } else {
            card= {id: this.props.nextTodayCard.details.id};
                    ...
                    ? <div className={classes.questionContainer} onMouseDown={() => {
                        ...
                        {this.props.nextTodayCard.loading ? <Loading/> :
                            <Typography color="inherit" variant="h5" component="h1">
                                {this.props.nextTodayCard.details.id>0 ? this.props.nextTodayCard.details.answer : 'No Card'}
                            </Typography>
                        }
                    </div>
                    : <div className={classes.questionContainer} onMouseDown={() => {
                        ...
                        {this.props.nextTodayCard.loading ? <Loading/> :
                            <Typography color="inherit" variant="h5" component="h1">
                                {this.props.nextTodayCard.details.id>0 ? this.props.nextTodayCard.details.question : 'No Card'}
                            </Typography>
                        }
                    </div>

                }
                {this.props.nextTodayCard.details.id > 0 ?
                    <div className={classes.buttonContainer}>
                        <Button className={classes.okBtn} onClick={() => {
                            this.onClickAnswerBtn(true)
                        }} variant="contained" color="primary">
                            I Know
                        </Button>
                        <Button className={classes.wrongBtn} onClick={() => {
                            this.onClickAnswerBtn(false)
