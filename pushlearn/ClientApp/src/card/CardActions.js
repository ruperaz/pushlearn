...
export function fetchCards (categoryId) {
    return dispatch => {
        ...
        return fetch(`${config.url.api}card/getall`, authHeader(localStorage.getItem('token'))).then(function (response) {
            if (response.ok) {
                response.json().then(function (response) {
                    dispatch({
                        type: SET_CARDS,
                        cards: (response.length > 0) ? response : []
                    })
                })
            } else {
...
export function fetchNextTodayCard (categoryId) {
    return dispatch => {
        ...
        if(categoryId){
            return fetch(`${config.url.api}card/getnexttodaycard?categoryId=${categoryId}`, authHeader(localStorage.getItem('token'))).then(function (response) {
                if (response.ok) {
                    response.json().then(function (response) {
                            dispatch({
                                type: SET_NEXT_TODAY_CARD,
                                nextTodayCard: (response && response.id) ? response : {}
                            })
                    })
                } else {
                    console.log('Looks like the response wasn\'t perfect, got status', response.status)
